    ...
    try:
        ...
        if min_Student >= no_of_apple:
            ...
            for i in range(min_Student, max_Student + 1):
                ...
                if rem == 0:
                    print(
                        f"{i} which is number between  min to max devisor of No. of apple\n"
                    )
                    print(
                        f"Apple is Equally distributed between each student \n Each student get {no_of_apple / i}\n"
                    )
                elif rem != 0:
                    print(
                        f"{i} which is number between  min to max is not devisor of No. of apple\n"
                    )
                    avg = no_of_apple / 2
                    if rem >= avg:
                        request_apple = int(
                            input(f"Please Give {no_of_apple - rem} more apple\n")
                        )
                        ...
                        print(
                            f"Hey My friend i am returning {rem} apples to you And Thank You\n"
                        )
