...
while True:
    ...
    try:
        test_inp = int(
            input(
                "Enter how many numberes you want to enter for getting Next palindrome:\n"
            )
        ...
        for i in range(test_inp):  # for loop for store inputs in list
            inp_list.append(
                input("Enter The Number you want to find the Next Palindrome:\n")
            )
        ...
        for lists in inp_list:  # this loop for iterate inputs one bye one .
            if lists == lists[::-1]:  # checking given input is palindrome or not
                ...
                while True:
                    ...
                    ):  # for checking palindrome in string
        ...
        print(
            "Please Enter Integer values You want to continue press [y] for exit press any key\n"
        )
        if input() == "y":
            continue
        else:
